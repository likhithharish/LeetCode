class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        if(nums1.length>nums2.length){
            //Changing references as this needs nums1 to be smaller.
            int[] temp = nums1;
            nums1 = nums2;
            nums2 = temp;
        }
        ...
        while(begin<= end){
            ...
            if(max1<=min2 && max2<=min1){
                ...
                //left side is small than right side
                if((n1+n2)%2==0){
                ...
                end = i1-1;  // shifting to left side
            }else{
                begin = i1+1; //shifting to right side
            }
        }
        return -1;
    }
